	...
	for part in message.walk():
		...
		if filename.find('.jpg') > 0 or filename.find('.jpeg') > 0 or filename.find('.png') > 0 or filename.find('.gif') > 0 or filename.find('.pdf') > 0:
			...
			if part.get('Content-ID'):
				...
				email_body = email_body + '<div class=\'holder\'><a href=\'assets/%s\'><img src=\'assets/%s\'></a></div>' % (filename, filename)
...
if uid_list[0] != '':
	for uid in uid_list:
		...
		try:
			...
			new_post = new_post.replace('{3}', dateutil.parser(post_date).strftime('%b %d, %Y')
